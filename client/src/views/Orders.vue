<template>
  <div class="orders">
    ...
    <div v-else>
      ...
      <div class="card" v-if="getOrdersByStatus('Submitted').length > 0">
...
export default {
  ...
  setup() {
    ...
    const getLeadTimeDays = (order) => {
      const now = new Date()
      const delivery = new Date(order.expected_delivery)
      const diffDays = Math.max(0, Math.ceil((delivery - now) / (1000 * 60 * 60 * 24)))
